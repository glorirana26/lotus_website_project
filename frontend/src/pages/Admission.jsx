...
export default function Admission() {
  ...
  const validateField = (field, value) => {
    let msg = "";
    if (field === "name" && value.length > 0 && value.length < 3) {
      msg = "Name must be at least 3 characters";
    }
    if (field === "mobile" && value.length > 0 && !isValidPhone(value)) {
      msg = "Enter valid 10 digit mobile number starting with 6-9";
    }
    if (field === "email" && value.length > 0 && !isValidEmail(value)) {
      msg = "Enter valid email address (e.g. abc@gmail.com)";
    }
    if (field === "percentage" && value.length > 0 && (isNaN(value) || value < 0 || value > 100)) {
      msg = "Enter valid percentage between 0-100";
    }
